...
import ClientPortalV2 from '@/components/v2/ClientPortalV2'
import PasswordGate from '@/components/PasswordGate'

// v2 client portal. Reuses the exact same token/password verification flow as the
// production /portal/[token] route — only the rendered UI differs.
export default function ClientPortalV2Page() {
  ...
  const verifyAccess = async (password = null) => {
    try {
      ...
      if (data.requiresPassword) {
        setClientPreview({ name: data.clientName, logo: data.clientLogo })
        setStatus('password')
        return false
      }
  ...
  if (status === 'verified' && client) {
    return <ClientPortalV2 client={client} onRefresh={handleRefresh} />
  }

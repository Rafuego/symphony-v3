...
export async function updateNotionPage({
  ...
  try {
    ...
    // Timeline is a Notion date-RANGE (start + end). The caller passes both
    // sides together (or both null to clear the field). When neither has been
    // set on the request yet (e.g. queued task with no requested date), skip.
    if (startedAt !== undefined || requestedDueDate !== undefined) {
      if (!startedAt && !requestedDueDate) {
        properties['Timeline'] = { date: null }
      } else if (startedAt && requestedDueDate) {
        properties['Timeline'] = { date: { start: startedAt, end: requestedDueDate } }
      } else if (startedAt) {
        // Only start known — use calculated default (2 business days) for end.
        properties['Timeline'] = { date: { start: startedAt, end: calculateDueDate(startedAt) } }
      } else {
        // Only end known (task hasn't started). Use the due date as both so
        // Notion has a valid pointer without inventing a fake start-of-today.
        properties['Timeline'] = { date: { start: requestedDueDate, end: requestedDueDate } }
      }
    }
